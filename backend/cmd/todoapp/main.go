...
import (
	...
	auth_postgres_repository "github.com/zzhassyn/todo-app/internal/features/auth/repository/postgres"
	...
	auth_worker "github.com/zzhassyn/todo-app/internal/features/auth/worker"
	...
	tasks_worker "github.com/zzhassyn/todo-app/internal/features/tasks/worker"
...
func main() {
	...
	authService := auth_service.NewAuthService(authRepository, usersService, auth_service.Config{
	...
	logger.Debug("starting recurring tasks worker")
	tasks_worker.StartRecurringTasksWorker(ctx, tasksRepository, time.Minute)
	...
	httpServer.RegisterHealthcheck(pool)

	logger.Debug("starting refresh token cleanup worker")
	auth_worker.StartRefreshTokenCleanupWorker(ctx, authRepository, time.Hour)
