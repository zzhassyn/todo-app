...
type UncompleteTaskResponse TaskDTOResponse

func (h *TasksHTTPHandler) UncompleteTask(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	task, err := h.tasksService.UncompleteTask(ctx, taskID, claims.UserID)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to uncomplete task")
		return
	}

	response := UncompleteTaskResponse(taskDTOFromDomain(task))
