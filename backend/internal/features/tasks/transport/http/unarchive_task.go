...
type UnarchiveTaskResponse TaskDTOResponse

func (h *TasksHTTPHandler) UnarchiveTask(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	task, err := h.tasksService.UnarchiveTask(ctx, taskID, claims.UserID)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to unarchive task")
		return
	}

	response := UnarchiveTaskResponse(taskDTOFromDomain(task))
