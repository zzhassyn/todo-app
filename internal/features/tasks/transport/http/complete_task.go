...
func (h *TasksHTTPHandler) CompleteTask(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	task, err := h.tasksService.CompleteTask(ctx, taskID, claims.UserID)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to complete task")
		return
	}

	response := CompleteTaskResponse(taskDTOFromDomain(task))
